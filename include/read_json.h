  ...
  std::vector<std::shared_ptr<Light> > & lights)
{
    ...
    std::vector<std::shared_ptr<Light> > & lights)
  {
    ...
    for(const json & jlight : j)
    {
      ...
      }else if(jlight["type"] == "point")
      {
        ...
        light->I = parse_Vector3d(jlight["color"]);
        light->isDim = jlight["isDim"];
        light->lightRadius = jlight["lightRadius"];
        lights.push_back(light);
      }
    }
  };
    ...
    std::vector<std::shared_ptr<Object> > & objects)
  {
    ...
    for(const json & jobj : j)
    {
      ...
      }else if(jobj["type"] == "rectPrism")
      {
        std::shared_ptr<RectPrism> rectPrism(new RectPrism());
        rectPrism->minPoint = parse_Vector3d(jobj["minPoint"]);
        rectPrism->maxPoint = parse_Vector3d(jobj["maxPoint"]);
        objects.push_back(rectPrism);
